 ...
 * 注意：热词只能在识别的时候会增加热词的识别权重，需要注意的是增加相应词条的识别率，但并不是绝对的，具体效果以您测试为准。 错误码链接：
 * https://www.xfyun.cn/doc/asr/voicedictation/API.html#%E9%94%99%E8%AF%AF%E7%A0%81
 * https://www.xfyun.cn/document/error-code （code返回错误码时必看） 语音听写流式WebAPI
...
function translation(count,ent,FROM,TO) {
	var type = $("input[name='cnonen']:checked").val();
	if(type == 1){
		ent="intp65";
		FROM="en";
		TO="cn"
